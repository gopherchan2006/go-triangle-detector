...
func (r *EChartsRenderer) RenderCandles(candles []domain.Candle) {
	r.candles = candles
...
func (r *EChartsRenderer) DrawScatterMarkers(points []detect.SwingPoint, label string, color string) {
	r.overlays = append(r.overlays, overlay{
		kind:   kindScatter,
		label:  label,
		color:  color,
